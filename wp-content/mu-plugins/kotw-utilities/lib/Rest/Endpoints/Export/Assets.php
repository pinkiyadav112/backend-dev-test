 ...
 *
 * @endpoint: /{wp-json}/kotw/exportDatabase
 *
 */
class Assets extends Endpoint {
	...
	public static function callback( WP_REST_Request $request ) {
		...
		$admin_request = false;
		// Check if the current user has the ability to view the requested endpoint.
		...
		$file_name = Helper::get_latest_export_file( 'assets' );
